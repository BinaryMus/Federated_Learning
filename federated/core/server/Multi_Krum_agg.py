...
class Multi_Krum(server.BaseServer):
    def __init__(self, epoch: int, clients: List, model: torch.nn.Module, data: DataLoader, device: str, f: int = 0,agg_numbers: int = 3):
        ...
        self.para_cache = []
        self.f = f
        self.agg_numbers = min(agg_numbers,self.n_clients)
    ...
    def pull(self, client_nums, total):
        ...
        for key in self.model.state_dict():
            dtype = self.clients[0].model.state_dict()[key].dtype
            for i in range(0,self.agg_numbers):
                self.model.state_dict()[key] += (1.0/self.agg_numbers) * (self.clients[self.scores[i][0]].model.state_dict()[key]).to(dtype)
