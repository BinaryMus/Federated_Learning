...
class Krum(server.BaseServer):
    ...
    def get_score_min(self):
        ...
        for i in range(self.n_clients):
            for j in range(i + 1, self.n_clients):
                totscore = 0.0
                for key in self.para_cache[0]:  # Assume para_cache[0] is non-empty
                    diff = self.para_cache[i][key] - self.para_cache[j][key]
                    squared_diff = (diff * diff).sum()
                    score = torch.sqrt(squared_diff)
                    totscore += score
                self.tmp_scores[i].append(totscore)
                self.tmp_scores[j].append(totscore)

        for i in range(self.n_clients):
            self.scores[i][0] = i
            self.scores[i][1] = torch.topk(torch.tensor(self.tmp_scores[i]), self.n_clients - self.f - 2, largest = False).values.sum()
            print("client #{} : {}".format(i, self.scores[i][1]))
    ...
    def pull(self, client_nums, total):
        ...
        print("choice #{}".format(self.scores[0][0]))
        for key in self.model.state_dict():
            dtype = self.clients[0].model.state_dict()[key].dtype
